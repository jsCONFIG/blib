...
define( function ( require ) {
    ...
    Class_db.prototype.checkData = function ( data ) {
        ...
        if( !this.primaryKey ) {
            ...
            // string操作
            priVal = data[ this.primaryKey ].toString();
    ...
    Class_db.prototype.del = function ( whereObj ) {
        ...
        for( var i = 0, pkL = primaryKeys.length; i < pkL; i++ ) {
            ...
            _eachObj( self.idx, function ( j, item ) {
                // 先检测该数据项是否存在当前的索引内容
                self.data[ key ].hasOwnProperty( j ) && item.del( self.data[ key ][ j ] );
            });
    ...
    Class_index.prototype.del = function ( key ) {
        key = key.toString();
    ...
    Class_index.prototype.update = function ( priVal, oldKey, newKey ) {
        this.dataIdx[ oldKey ] && this.dataIdx[ oldKey ].del( priVal );
